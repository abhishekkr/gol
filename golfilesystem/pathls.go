...
It need to be provided path and returns mapping like following example.
{
	"/a": ["/a/b"],
...
*/
func PathLs(basePath string) (map[string]map[int]string, error) {
...
*/
func PathLsN(basePath string, depth int) (map[string]map[int]string, error) {
...
*/
func PathLsType(basePath string, filetypes []string) (map[string]map[int]string, error) {
...
*/
func PathLsNType(basePath string, depth int, filetypes []string) (map[string]map[int]string, error) {
...
The filetype list could look like [".mp4", ".avi"]
*/
func PathLsTypeNot(basePath string, filetypes []string) (map[string]map[int]string, error) {
...
*/
func PathLsNTypeNot(basePath string, depth int, filetypes []string) (map[string]map[int]string, error) {
...
*/
func doPathWalk(dirPath string, recurseDepth int, filetypes []string, filetypeBool bool) (map[string]map[int]string, error) {
	var pathMap = make(map[string]map[int]string)
...
*/
func walkFile(root string, path string, fi os.FileInfo, err error, recurseDepth int, filetypes []string, filetypeBool bool, pathMap *map[string]map[int]string) error {
	if fi.IsDir() {
		return nil
	}
	...
	if isMedia(filepath.Ext(path), filetypes) == filetypeBool {
		if len((*pathMap)[root]) == 0 {
			(*pathMap)[root] = make(map[int]string)
		}
		(*pathMap)[root][len((*pathMap)[root])] = path
	}
	return nil
}
